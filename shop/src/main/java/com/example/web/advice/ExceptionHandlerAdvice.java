...
/*
 * 	@ControllerAdvice
 * 		- 여러 컨트롤러 클래스에서 공통으로 사용되는 기능이 정의된 클래스임을 나타내는 어노테이션이다.
 ...
 * 				+ 발생한 예외를 처리할 수 있는 예외처리 메소드가 여러 개일 때는 발생한 예외와
 * 					가까운 예외가 정의된 예외처리 햄들러 메소드가 실행된다.
 * 				+ 예외처리 핸들러 메소드는 매개변수를 통해서 발생한 예외를 전달받을 수 있다.
 * 		  @InitBinder
 * 				+ 컨트롤러로 전달되는 요청 파라미터에 대한 추가적인 작업을 수행하는 메소드임을 나타내는 어노테이션이다.
 * 		  @ModelAttribute
 * 				+ 메소드에 붙일 수 있고, 파라미터에 붙일 수 있다.
 * 		
 */
@ControllerAdvice
@Log4j2
public class ExceptionHandlerAdvice {
	...
	// Exception 중 더 구체적인 Exception에 매핑된다.
	// DataAccess중 오류가 발생하면 Exception이 아닌 좀 더 구체적인 DataAccessException에 매핑된다.
