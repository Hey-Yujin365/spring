...
/*
		http
			...
			.authorizeHttpRequests((authorizeHttpRequests) -> authorizeHttpRequests
					.antMatchers("/resources/**").permitAll()				// "/resources" 및 하위경로에 대해서는 접근을 허용한다.
					.antMatchers("/", "/login", "/register").permitAll()	// "/", "/login", "/register"에 대해서는 접근을 허용한다.
...
*/
		...
		http
			...
			.formLogin((formLogin) -> formLogin
					...
					.failureUrl("/login?error=fail"));	// xxxx로그인에 실패하면 "/login/error"를 재요청 URL로 지정한다.
														// 수정함, "/login?error=fail"로 바꿔야 로그인 오류났을 때 접근가능해진다.
		...
		http
			...
			// 인증, 인가 과정에서 발생하는 예외를 처리하는 사용자정의 핸들러 구현해서 등록할 수 있으면,
			// 따로 설정하지 않으면 기본으로 등록되어있는 핸들러가 동작한다.
