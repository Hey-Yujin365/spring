 ...
 * 
 * @Id
 * 		- 해당 프로퍼티는 엔티티를 식별하는 값을 포함하고 있는 프로퍼티임을 나타낸다.
 * 		- 이 어노테이션이 지정된 프로퍼티는 기본키 컬름의 값을 표현하는 프로퍼티다.
 * 
 * @GeneratedValue
 ...
 * 				+ oracle에서 auto로 지정하면 table방식을 사용한다.
 * 
 * @Enumerated
 * 		- 이 어노테이션은 상수데이터의 주요한 값(순번과 이름) 중에서 어떤 값을 엔티티에서 사용할 것인지 지정하는 어노테이션이다.
